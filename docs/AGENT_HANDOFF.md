...
- **Gate**: GA4 + Clarity scripts load only when `NEXT_PUBLIC_DEPLOY_CONTEXT === "production"` (`src/app/layout.tsx`), set per-context in `netlify.toml`. Netlify's own `CONTEXT` cannot be used: it is build-only and never inlined, so ISR re-renders in the runtime function dropped both tags. IDs come from `NEXT_PUBLIC_GA_MEASUREMENT_ID` / `NEXT_PUBLIC_CLARITY_PROJECT_ID` (set in Netlify env; empty locally). `NEXT_PUBLIC_*` is inlined at build time — a missing var silently disables that tag.
...
- **Audience**: `trackEvent()` stamps an `audience` of `schools` / `funders` / `tutors` / `general`, derived from the pathname (`audienceForPath` in `src/lib/analytics.ts` is the single source of truth). Separate from, and coarser than, Clarity's `page_type`. A user-scoped `first_audience` records the door the visitor entered through — session-scoped and write-once, seeded pre-hydration and topped up by `first-audience-tracker.tsx` on soft navigations. Attribution questions need `first_audience`; page-level questions need `audience`.
- **Scroll depth**: `scroll-depth-tracker.tsx` fires `scroll_depth` at 25/50/75/90 with `percent_scrolled`. Named that way because `scroll` is GA4-reserved. Re-arms per route change; pages that fit on screen report nothing.
- **Form submissions**: `/thanks` fires `form_submit` from `?form=contact|tutor|demo|school`. **Read the next paragraph before relying on it.** Google Forms cannot redirect anywhere after submission — the confirmation setting is a *message*, not a URL, and the Forms API exposes no confirmation field at all (`FormSettings` has only `emailCollectionType` and `quizSettings`). The most a form can do is show a clickable link to `/thanks`, which respondents have to notice and click. So `form_submit` counts "submitted **and** clicked through", not "submitted", and undercounts by an unknown margin. Do not read it as a completion count.
  Counting iframe `load` events on the embedded form does not rescue this either: Form A ("Contact Form") has four page breaks, so every "Next" fires a load. The ground truth for completions is each form's linked response sheet, which nothing here reads yet.
- **Campaign tagging**: every hand-placed inbound link carries `utm_source` / `utm_medium` / `utm_campaign`, with `utm_medium` drawn from a closed five-value list. Spec and worked examples: `docs/utm-conventions.md`.
- **GA4 configuration**: custom dimensions, key events and the internal-traffic filter are recorded in `docs/ga4-configuration.md`. Custom dimensions are **not retroactive** — register before shipping an event, never after.
- **Clarity**: project `kumnxwffl4` (the original — `x3ycp8d2ay` was a dev-URL throwaway). `clarity-tagger.tsx` sets `page_type` per route change; click handler also fires `cta_*` Clarity events + `upgrade`. Smart-event and funnel spec: `docs/clarity-events-and-funnels.md`.
- **Internal traffic**: GA4's IP-based rule is unreachable (its Admin UI panel is broken and it has no API), so `trackEvent()` stamps `traffic_type: "internal"` from a localStorage flag instead. Teammates opt in per browser via `https://tutors.plus/?internal=1` (`?internal=0` to undo). The built-in GA4 Internal Traffic data filter excludes on that parameter.
- **Referral exclusion**: `instrumentation-client.ts` pushes `ignore_referrer` when the referrer is a Google Forms / accounts host, so returns from an outbound form don't open a new session attributed to `docs.google.com`. Same reason — the GA4 UI equivalent is unreachable.
...
- **Redirects**: legacy Framer URLs redirect in `next.config.ts` `redirects()`. The map is **generated** — one list of pairs emits both the bare and `/en`-prefixed form, which is what keeps every hop single. A blanket `/en/:path*` rule looks equivalent and is not: it sends `/en/about/story` to `/about/story`, which is itself a redirect. `src/lib/redirects.test.ts` enforces no-chaining. Paths with no destination (Terms, Privacy, FAQ, release notes) are deliberately unmapped.
