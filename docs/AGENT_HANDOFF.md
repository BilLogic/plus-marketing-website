...
- **Form submissions**: `/thanks` is the confirmation destination for our outbound Google Forms — `?form=contact|tutor|demo|school` fires `form_submit`. Holding it alongside the `*_click` events makes the form's own drop-off measurable for the first time. Requires each Google Form's confirmation URL to point there.
